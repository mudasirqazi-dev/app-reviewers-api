...
router.post('/login', async (req, res) => {
  ...
  if (user.isVerified == false) return res.status(400).send({
    success: false,
    message: 'please verify your email address'
  });
  if (user.isDeleted == true) return res.status(400).send({
    success: false,
    message: 'your account is blocked please contact with admin'
  });
  if (user.isBlock == true) return res.status(400).send({
    success: false,
    message: 'your account is blocked please contact with admin'
  });
...
router.post('/resetforgotPassword', async (req, res) => {
  let token = req.body.token;
  jwt.verify(token, process.env.Encription_Secret, async function (err, data) {
    if (err || !data) {
      res.status(500).json({
