...
const Manager = {
      ...
      return await PaymentModel.aggregate([
        {
          $match: {
            date: {
              $gte: new Date(moment().month(month).year(year).startOf("month")),
              $lte: new Date(moment().month(month).year(year).endOf("month")),
            },
          },
        },
        {
  ...
  getUserPayments: async (userId, month, year, interval) => {
    if (interval === "daily")
      return await PaymentModel.aggregate([
        {
          $match: {
  ...
  getUserSearches: async (userId, month, year, interval) => {
    if (interval === "daily")
      return await SearchModel.aggregate([
        {
          $match: {
